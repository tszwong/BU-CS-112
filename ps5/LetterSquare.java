...
public class LetterSquare {
    ...
    private boolean isValid(String letter, int wordNum, int charNum) {        
        ...
        if ((charNum == 0 && wordNum != 0) && ((lastLetter(words[wordNum-1]).equals(letter)))) {
            return true;
        } else if (wordNum == 0 && charNum == 0) {
            return true;
        } else if ((this.onSameSide(words[wordNum], letter) != true) && dictionary.hasString(words[wordNum] + letter)) {
            if (alreadyUsed(words[wordNum] + letter) != true) {
                return true;
            }
        } 
    ...
    private boolean solveRB(int wordNum, int charNum, int maxWords) {
        if (this.allLettersUsed() && dictionary.hasFullWord(words[wordNum])) {   // base case
            this.printSolution(wordNum);
            return true;
        } else if (wordNum >= maxWords) {   // base case 
            return false;
        } else {   // recursive case 
            for (int i = 0; i < letters.length; i++) {   
                if (this.isValid(letters[i], wordNum, charNum)) {   // check if valid
                    this.addLetter(letters[i], wordNum);   // add the letter to letters
                    if (solveRB(wordNum, (charNum + 1), maxWords) == true) {   // check if next call meets base case
                        return true;
                    } else if ((words[wordNum].length() >= 3 && dictionary.hasFullWord(words[wordNum])) && solveRB(wordNum + 1, 0, maxWords)) {
                        return true;
                    } else {
